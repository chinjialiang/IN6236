...
contract Election {
    ...
    constructor(
        ...
        require(
            candidateNames.length == candidatePartyNames.length &&
                candidateNames.length == candidatePartyImages.length,
            "Candidates information length mismatch."
        );
    ...
    function getElectionInformation()
        public
        view
        returns (string memory, string memory, string memory, uint, uint, uint)
    {
        return (
            electionName,
            description,
    ...
    function getCandidate(
        ...
        Types.Candidate memory candidate = candidates[_index];
        return (
            candidate.name,
            candidate.partyName,
        ...
        returns (string memory, string memory, string memory, uint)
    {
        ...
        require(
            isWinnerDetermined == true,
            "The winner has not been determined, thank you for your patience"
        );
        // if isWinnerDetermined is true and winnerIndex is out of bound, nobody has voted
        require(
            winnerIndex != type(uint).max,
            "Apprently Nobody has voted! Please wait for official announcement"
        );

        // return information of winning candidates
        Types.Candidate memory candidate = candidates[winnerIndex];
        return (
            candidate.name,
            candidate.partyName,
            candidate.partyImage,
